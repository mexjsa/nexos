// ============================================================
// FIREBASE CONFIG — Proyecto calle9-2ca66
// Leads: real_estate_leads (CALLE9), nexos_leads (NEXOS)
// ============================================================
...
// ✅ FIREBASE REAL — Guarda lead de CALLE9 en Firestore
async function submitReLead() {
    const name = document.getElementById('re-form-name').value.trim();
    const phone = document.getElementById('re-form-phone').value.trim();
    const email = document.getElementById('re-form-email').value.trim();
    const date = document.getElementById('re-form-date').value.trim();
    ...
    const btn = document.querySelector('#re-form-overlay button[onclick="submitReLead()"]');
    btn.innerText = "Guardando...";
    btn.style.opacity = "0.7";
    btn.disabled = true;

    try {
        await addDoc(collection(_db, 'real_estate_leads'), {
            name,
            phone,
            email,
        ...
        console.error('Error guardando lead CALLE9:', err);
...
function renderMedCorpDemo() {
    demoContainer.innerHTML = `
        <div style="display: flex; flex-direction: column; gap: 1rem;">
            <!-- Header -->
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <div>
                    <h3 style="margin-bottom: 0.2rem; color: #06b6d4;">Med&amp;Corp | Motor de Extracción v2.1</h3>
                    <p style="font-size: 0.8rem; color: #a1a1aa;">Sistema de inteligencia documental para resultados clínicos de laboratorio.</p>
                </div>
                <div style="background: rgba(6,182,212,0.1); padding: 0.4rem 0.8rem; border-radius: 20px; border: 1px solid rgba(6,182,212,0.3);">
                    <span style="color: #06b6d4; font-size: 0.75rem; font-weight: bold;">🏥 MODO DEMO</span>
                </div>
            </div>

            <!-- Two-column layout -->
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; height: 420px;">

                <!-- Left: PDF Viewer Mockup + Controls -->
                <div style="display: flex; flex-direction: column; gap: 1rem;">
                    <!-- Fake PDF -->
                    <div style="background: #fff; color: #111; border-radius: 8px; padding: 1.2rem; font-family: monospace; font-size: 0.7rem; flex: 1; overflow: hidden; position: relative;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.8rem; border-bottom: 1px solid #ddd; padding-bottom: 0.5rem;">
                            <strong style="font-size: 0.85rem;">MED&amp;CORP LABORATORIO</strong>
                            <span style="font-size: 0.65rem; color:#666;">Folio: MC-2026-0423</span>
                        </div>
                        <div style="color:#333; line-height: 1.6;">
                            <p><strong>Paciente:</strong> [PROTEGIDO]</p>
                            <p><strong>Fecha:</strong> 23/04/2026</p>
                            <div style="margin-top: 0.8rem; border: 1px solid #ddd; border-radius: 4px; overflow: hidden;">
                                <table style="width:100%; border-collapse: collapse; font-size: 0.65rem;">
                                    <tr style="background:#f0f0f0;"><th style="padding:3px 6px; text-align:left;">Estudio</th><th style="padding:3px 6px;">Resultado</th><th style="padding:3px 6px;">Ref.</th></tr>
                                    <tr><td style="padding:3px 6px;">Glucosa</td><td style="padding:3px 6px; text-align:center;">98 mg/dL</td><td style="padding:3px 6px; text-align:center;">70–100</td></tr>
                                    <tr style="background:#fafafa;"><td style="padding:3px 6px;">Hemoglobina</td><td style="padding:3px 6px; text-align:center;">13.8 g/dL</td><td style="padding:3px 6px; text-align:center;">12–16</td></tr>
                                    <tr><td style="padding:3px 6px;">Colesterol Total</td><td style="padding:3px 6px; text-align:center; color:#b91c1c; font-weight:bold;">215 mg/dL</td><td style="padding:3px 6px; text-align:center;">&lt;200</td></tr>
                                    <tr style="background:#fafafa;"><td style="padding:3px 6px;">Triglicéridos</td><td style="padding:3px 6px; text-align:center;">142 mg/dL</td><td style="padding:3px 6px; text-align:center;">&lt;150</td></tr>
                                    <tr><td style="padding:3px 6px;">Creatinina</td><td style="padding:3px 6px; text-align:center;">0.9 mg/dL</td><td style="padding:3px 6px; text-align:center;">0.7–1.2</td></tr>
                                </table>
                            </div>
                        </div>
                        <div id="mc-pdf-highlight" style="position: absolute; top:0; left:0; width:100%; height:100%; pointer-events:none; display:none;"
                             ></div>
                    </div>

                    <!-- Trigger button -->
                    <button id="mc-run-btn" onclick="runMedCorpExtraction()" class="btn btn-primary"
                        style="width: 100%; background: linear-gradient(135deg, #0ea5e9, #06b6d4); font-weight: 700; letter-spacing: 0.5px;">
                        ⚡ Ejecutar Motor de Extracción
                    </button>
                </div>

                <!-- Right: Terminal + Excel Output -->
                <div style="display: flex; flex-direction: column; gap: 1rem; height: 100%;">
                    <!-- Terminal -->
                    <div id="mc-terminal" style="background: #000; padding: 1rem; font-family: 'Courier New', monospace; font-size: 0.7rem; flex: 1; overflow-y: auto; color: #06b6d4; border-radius: 8px; border: 1px solid rgba(6,182,212,0.2);">
                        <div style="color:#4ade80;">Med&amp;Corp Motor v2.1 — listo.</div>
                        <div style="color:#71717a;">Esperando instrucción...</div>
                    </div>

                    <!-- Excel Output -->
                    <div id="mc-excel-output" style="display:none; background: rgba(16,185,129,0.05); border: 1px solid rgba(16,185,129,0.3); border-radius: 8px; padding: 0.8rem;">
                        <div style="font-size:0.75rem; color:#10b981; font-weight:bold; margin-bottom:0.5rem;">✅ Reporte Excel Generado</div>
                        <table style="width:100%; border-collapse:collapse; font-size:0.65rem; color:#e4e4e7;">
                            <tr style="background:rgba(16,185,129,0.15); color:#10b981;">
                                <th style="padding:4px 6px; text-align:left;">Estudio</th>
                                <th style="padding:4px 6px; text-align:center;">Valor</th>
                                <th style="padding:4px 6px; text-align:center;">Status</th>
                            </tr>
                            <tr><td style="padding:3px 6px;">Glucosa</td><td style="padding:3px 6px; text-align:center;">98</td><td style="padding:3px 6px; text-align:center; color:#4ade80;">✓ Normal</td></tr>
                            <tr style="background:rgba(255,255,255,0.03);"><td style="padding:3px 6px;">Hemoglobina</td><td style="padding:3px 6px; text-align:center;">13.8</td><td style="padding:3px 6px; text-align:center; color:#4ade80;">✓ Normal</td></tr>
                            <tr><td style="padding:3px 6px;">Colesterol Total</td><td style="padding:3px 6px; text-align:center;">215</td><td style="padding:3px 6px; text-align:center; color:#f87171;">⚠ Alto</td></tr>
                            <tr style="background:rgba(255,255,255,0.03);"><td style="padding:3px 6px;">Triglicéridos</td><td style="padding:3px 6px; text-align:center;">142</td><td style="padding:3px 6px; text-align:center; color:#4ade80;">✓ Normal</td></tr>
                            <tr><td style="padding:3px 6px;">Creatinina</td><td style="padding:3px 6px; text-align:center;">0.9</td><td style="padding:3px 6px; text-align:center; color:#4ade80;">✓ Normal</td></tr>
                        </table>
                        <div style="margin-top:0.8rem; text-align:right;">
                            <span style="font-size:0.65rem; color:#06b6d4; background:rgba(6,182,212,0.1); padding: 3px 8px; border-radius:4px;">📊 check_up_med.xlsx — listo para descarga</span>
                        </div>
                    </div>
                </div>
            </div>
...
function runMedCorpExtraction() {
    const terminal = document.getElementById('mc-terminal');
    const btn = document.getElementById('mc-run-btn');
    const excelOutput = document.getElementById('mc-excel-output');

    btn.disabled = true;
    btn.innerText = '⏳ Procesando...';
    excelOutput.style.display = 'none';
    terminal.innerHTML = '<div style="color:#4ade80;">Med&amp;Corp Motor v2.1 — iniciando extracción...</div>';

    const steps = [
        { text: '> Leyendo archivo: MC-2026-0423.pdf', color: '#06b6d4', delay: 400 },
        { text: '> Detectando estructura de tablas...', color: '#a1a1aa', delay: 900 },
        { text: '> Layout identificado: 3 columnas (Estudio | Resultado | Referencia)', color: '#a1a1aa', delay: 1500 },
        { text: '> Extrayendo fila 1: Glucosa → 98 mg/dL ✓', color: '#4ade80', delay: 2100 },
        { text: '> Extrayendo fila 2: Hemoglobina → 13.8 g/dL ✓', color: '#4ade80', delay: 2600 },
        { text: '> Extrayendo fila 3: Colesterol Total → 215 mg/dL ⚠ FUERA DE RANGO', color: '#f87171', delay: 3100 },
        { text: '> Extrayendo fila 4: Triglicéridos → 142 mg/dL ✓', color: '#4ade80', delay: 3600 },
        { text: '> Extrayendo fila 5: Creatinina → 0.9 mg/dL ✓', color: '#4ade80', delay: 4100 },
        { text: '> Cargando plantilla check_up_med.xlsx...', color: '#a1a1aa', delay: 4700 },
        { text: '> Mapeando columnas DC:FV → 5 campos escritos.', color: '#a1a1aa', delay: 5200 },
        { text: '> Guardando: check_up_med_20260423_114500.xlsx', color: '#06b6d4', delay: 5800 },
        { text: '✅ Extracción completada. 0 errores. Tiempo: 1.4s', color: '#4ade80', delay: 6300 },
    ];
    ...
    // Show Excel output after all steps
    setTimeout(() => {
        excelOutput.style.display = 'block';
        btn.innerText = '⚡ Ejecutar Motor de Extracción';
        btn.disabled = false;
    }, 6700);
}
...
// handlers in HTML when portfolio.js is loaded as type="module")
// ============================================================
window.openDemo = openDemo;
window.closeDemo = closeDemo;
window.calculateRisk = calculateRisk;
window.setGunsView = setGunsView;
window.calculateInsurance = calculateInsurance;
window.setReLanguage = setReLanguage;
window.nextReImage = nextReImage;
window.prevReImage = prevReImage;
window.showReForm = showReForm;
window.hideReForm = hideReForm;
window.handleReChat = handleReChat;
window.submitReLead = submitReLead;
window.updateYRConversion = updateYRConversion;
window.runMedCorpExtraction = runMedCorpExtraction;
window.toggleNexosForm = toggleNexosForm;
window.submitNexosLead = submitNexosLead;

// ============================================================
// NEXOS IA — Intake Form Logic
// ============================================================

function toggleNexosForm() {
    const form = document.getElementById('nexos-intake-form');
    const btn  = document.getElementById('nexos-form-toggle');
    ...
    if (isHidden) {
        ...
        requestAnimationFrame(() => { form.style.animation = 'slideDown 0.4s ease'; });
...
async function submitNexosLead() {
    const name     = document.getElementById('nx-name').value.trim();
    const company  = document.getElementById('nx-company').value.trim();
    const email    = document.getElementById('nx-email').value.trim();
    const phone    = document.getElementById('nx-phone').value.trim();
    const budget   = document.getElementById('nx-budget').value;
    const desc     = document.getElementById('nx-desc').value.trim();
    const errorDiv = document.getElementById('nx-form-error');

    // Collect checked services
    const services = [...document.querySelectorAll('.nx-service:checked')].map(cb => cb.value);

    // Validate required fields
    errorDiv.style.display = 'none';
    if (!name) { errorDiv.innerText = '⚠ Por favor ingresa tu nombre.'; errorDiv.style.display = 'block'; return; }
    if (!email) { errorDiv.innerText = '⚠ Tu correo electrónico es requerido.'; errorDiv.style.display = 'block'; return; }
