...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const flow = {
        ...
        goto_form: {
            message: "¡Perfecto! Te llevo al formulario para que nos dejes tus datos y podamos preparar una propuesta. 📝",
            action: "scroll_to_form",
            options: []
        },
        direct_wa: {
            message: "Haz clic abajo para abrir WhatsApp y platicar directo con nuestro equipo. ¡Te esperamos! 👋",
        ...
        default: {
            message: "No estoy seguro de entender. ¿Podrías elegir una opción?",
            options: [
    ...
    chatWidget.innerHTML = `
        <div class="chat-button" id="chatButton" role="button" aria-label="Abrir asistente">
            <div class="chat-face">
                <div class="eyes">
        ...
        <div class="chat-window" id="chatWindow" role="dialog" aria-label="Nexo Assistant">
            <div class="chat-header">
                <h3>${config.botName}</h3>
                <button id="closeChat" type="button" aria-label="Cerrar asistente">×</button>
            </div>
            <div class="chat-messages" id="chatMessages"></div>
            <div class="chat-input-area" id="inputArea" style="display:none;">
                <input type="text" id="chatInput" placeholder="Escribe aquí..." aria-label="Mensaje">
                <button id="sendMessage" type="button" aria-label="Enviar">➤</button>
            </div>
        </div>
    `;
    ...
    function processNode(nodeKey) {
        ...
        setTimeout(() => {
            ...
            if (node.action === 'scroll_to_form') {
                // New Codez site: form is always visible, just scroll and focus
                setTimeout(() => {
                    document.querySelector('#contacto')?.scrollIntoView({ behavior: 'smooth' });
                    setTimeout(() => document.getElementById('nx-name')?.focus(), 600);
                }, 400);
                ...
                setTimeout(() => {
                    ...
                    waLink.rel = 'noopener';
                    waLink.style.cssText = 'display:inline-flex;align-items:center;gap:6px;margin-top:8px;padding:10px 16px;background:#1a7655;color:white;border-radius:4px;font-weight:700;font-size:.84rem;text-decoration:none;';
    ...
    function handleInput() {
        ...
        if (node.validate === 'email') {
            if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(text)) { alert("Por favor ingresa un correo válido."); return; }
        }
    ...
    function submitLeadData(data) {
        ...
        const lines = [
            ...
            data.business_size ? `*Tamaño del negocio:* ${sizeLabels[data.business_size] || data.business_size}`   : null,
            data.whatsapp      ? `*WhatsApp:* ${data.whatsapp}`                                                     : null,
            data.phone         ? `*Teléfono:* ${data.phone}`                                                        : null,
            data.email         ? `*Correo:* ${data.email}`                                                          : null,
            data.preferred_schedule ? `*Horario preferido:* ${scheduleLabels[data.preferred_schedule] || data.preferred_schedule}` : null,
        ...
        // Green button fallback inside chat
        setTimeout(() => {
            const waLink = document.createElement('a');
            waLink.href = waUrl;
            waLink.target = '_blank';
            waLink.rel = 'noopener';
            waLink.style.cssText = 'display:inline-flex;align-items:center;gap:6px;margin-top:10px;padding:12px 18px;background:#1a7655;color:white;border-radius:4px;font-weight:700;font-size:.88rem;text-decoration:none;';
